...
- `src/blackdog/ui.py:1502` builds the full UI snapshot by loading backlog, state, inbox, events, task results, threads, tracked installs, worktree contract data, and git metadata in one place.
- `src/blackdog/ui.py:1793` renders the static HTML page from that same snapshot and inlines both CSS and the snapshot JSON payload into the output file.
- `src/blackdog/ui.py:1515` through `src/blackdog/ui.py:1525` add git-origin and commit metadata directly inside the snapshot builder, which means the viewer surface is currently responsible for repo introspection as well as presentation.

Risk:

- A clean extraction cannot move only the HTML template. The data-shaping contract the page depends on is also in `ui.py`.
...
### 3. Supervisor extraction is high risk because launch policy, child protocol, and run artifacts are all implemented in one core module.
...
- `editors/emacs/lisp/blackdog.el:14` through `editors/emacs/lisp/blackdog.el:24` pull in Blackdog-specific dashboard, results, task, run, Codex, thread, spec, and telemetry modules from one package entrypoint.
- `editors/emacs/lisp/blackdog.el:27` through `editors/emacs/lisp/blackdog.el:54` hardcode inspection paths under `.codex/skills/blackdog/...`.
- `editors/emacs/lisp/blackdog.el:85` through `editors/emacs/lisp/blackdog.el:120` publish a top-level command map that treats dashboard, telemetry, worktree launch, and supervisor controls as one integrated product surface.
...
- A viewer or supervisor extraction that changes those paths or command outputs will break Emacs without any code move in `editors/emacs/`.
