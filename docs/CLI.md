...
- `blackdog plan`
- `blackdog summary [--focus-task TASK_ID ...]`
- `blackdog next [--focus-task TASK_ID ...]`
- `blackdog prompt`
- `blackdog thread new|list|show|append|prompt|task`
- `blackdog task edit|run`
- `blackdog tune`
...
- `blackdog supervise status [--focus-task TASK_ID ...]`
...
`blackdog plan` remains the legacy `epic` / `lane` / `wave` compatibility view over the backlog. Treat `runtime_snapshot.workset`, the task DAG, and task-state/attempt projections as the preferred runtime model; use `plan` when a human wants the current execution-map rendering, not when an integration needs canonical planning truth.

`blackdog summary` is the backlog-facing status view. It reports runnable work plus open inbox rows that still look actionable from the backlog's point of view, and it intentionally filters out raw supervisor-runtime transport or warning messages for completed/unknown tasks. Use `--focus-task TASK_ID` one or more times when you want a bounded workset slice instead of the full backlog: the selector is request-scoped, Blackdog derives the visible slice from those task ids, and prerequisite/lower-wave blocker semantics stay coherent inside that slice without mutating durable backlog state. Use `blackdog inbox list` or `blackdog snapshot` when you need the full append-only inbox state instead of the summary's filtered view.
...
`blackdog supervise run` is the draining supervisor mode. Conceptually it is a `WorksetExecution`; current persisted artifacts still use `run_id` and `supervisor-runs/` as compatibility lineage. The command performs one cleanup sweep at the start of the execution, removes already-completed tasks from the execution map, drops empty lanes and waves from the legacy plan projection, compacts remaining waves back to small integers, then keeps rereading backlog/state while the execution is active. Before it launches new child work from an idle point in that loop, it runs the primary-worktree recovery gate described above and records any resulting `stash`, `land`, or `commit` recovery actions in the execution payload. Tasks completed during that active execution stay visible in place on the execution map until the next run starts and sweeps them away. If there is no runnable or running work after that opening sweep, the command returns `idle` immediately instead of waiting for future tasks. Otherwise the execution exits only when it reaches idle or when a `stop` inbox message puts it into draining mode. `stop` prevents new launches but does not interrupt already-running child tasks.
...
`blackdog supervise status` is the chat-native inspection surface for the current supervisor execution. It reports the latest saved compatibility run status for a supervisor actor, the resolved WTAM workspace contract, the current launch defaults (including repo-owned model/reasoning policy and dynamic reasoning summaries when configured), and when the latest `status.json` captured them, the actual launch command/settings used for that execution. It also surfaces the derived `workset_execution` projection, the current pre-launch recovery decision when one exists, an explicit `recovery_needed` section for child outcomes that still require follow-up, the currently open control messages for that actor, the current ready-task queue, the active task-attempt rows and open wait-condition rows Blackdog has persisted, and the most recent supervisor or child-agent task results in one compact text or JSON view. Pass `--focus-task TASK_ID` to shrink that inspection to one bounded workset slice without changing durable backlog membership. Recovery-needed rows include the recoverable case plus the latest child and supervisor result summaries for that task/run when those artifacts exist.
...
- use `runtime_snapshot.project_*`, `runtime_snapshot.headers`, `runtime_snapshot.counts`, `runtime_snapshot.objectives`, `runtime_snapshot.next_rows`, `runtime_snapshot.open_messages`, `runtime_snapshot.plan`, `runtime_snapshot.workset`, `runtime_snapshot.task_dag`, `runtime_snapshot.tasks[*]`, and the additive lineage projections when you need attempt/wait/control visibility
...
`blackdog render` writes the static repo-branded backlog HTML page under the configured control root and refreshes the compatibility `backlog-index.html` copy beside it. Blackdog CLI writes and active supervisor runs rerender that page as part of normal state changes, including supervisor exit after landed task-state updates. The page embeds the current snapshot JSON directly, renders a wider control/status top band, a dedicated unattended-tuning panel sourced from `tracked-installs.json` plus `blackdog tune` analysis, the live execution map, and a completed-task card list, keeps artifact navigation as plain links, and opens execution/history cards in the task reader. When a child run captured `stdout.log` and a landed commit, the reader also shows the inline model response plus a landed-commit link or message. Operators can still reload the file manually, and the hero header now includes an optional 30-second auto-reload toggle with a visible countdown.
...
- actual resolved launch policy (`latest_run.launch_settings`, `runs[*].launch_settings`,
