...
`repo install`, or `repo update`. The `AGENTS.md` section and repo-local skill
are regenerated by `repo refresh`. `repo scaffold` does not create a durable
scaffold spec file; it creates or initializes the target git repo, may copy
starter docs from an exemplar, then writes the normal repo-local contract.
Generated repo contracts require a `git status --short` closeout after
`repo install`, `repo update`, or `repo refresh`; repo-visible managed file
changes are not durable until committed, landed, reverted, or explicitly
reported.
