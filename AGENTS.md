...
- Treat kept implementation edits in the primary worktree as a contract violation. Before any repo edit you intend to keep, run `./.VE/bin/blackdog worktree preflight`; if it reports `primary worktree: yes`, stop and create or enter a branch-backed task worktree before touching repo files.
...
- Keep `[taxonomy].doc_routing_defaults` pointed at the docs agents must review before editing, and refresh the generated project skill when that routing changes so the repo contract stays explicit.
