...
from blackdog_core.backlog import BacklogError, BacklogTask, load_backlog
...
def _task_for_id(profile: RepoProfile, task_id: str) -> BacklogTask:
...
def _task_slug(task: BacklogTask) -> str:
    return slugify(f"{task.id}-{task.title}")


def default_task_branch(task: BacklogTask) -> str:
    return f"agent/{_task_slug(task)}"


def task_id_for_branch(profile: RepoProfile, branch: str | None) -> str | None:
...
def default_task_worktree_path(profile: RepoProfile, task: BacklogTask) -> Path:
    return profile.paths.worktrees_dir / f"wt-{_task_slug(task)}"


def supervisor_task_branch(task: BacklogTask, run_id: str) -> str:
    return f"{default_task_branch(task)}-{run_id}"


def supervisor_task_worktree_path(profile: RepoProfile, task: BacklogTask, run_id: str) -> Path:
