...
from blackdog_core.backlog import render_initial_backlog, refresh_backlog_headers
...
from blackdog_core.state import append_event, default_state, save_state
from .board import build_board_snapshot, render_static_html
...
def _ensure_runtime_dirs(profile: RepoProfile) -> None:
...
def _profile_for_paths(profile: RepoProfile, *, paths) -> RepoProfile:
...
def _ensure_baseline_agents_file(project_root: Path) -> None:
    ...
    agents_file.write_text(
        ...
        "- Use repository-local `./.VE/bin/blackdog` when present.\n"
...
def scaffold_named_backlog(profile: RepoProfile, name: str, *, force: bool = False) -> Path:
...
def remove_named_backlog(profile: RepoProfile, name: str) -> Path:
    paths = named_backlog_paths(profile, name)
...
def reset_default_backlog(profile: RepoProfile, *, purge_named: bool = False) -> Path:
...
def create_project(
    ...
    *,
    ...
    evidence_requirements: list[str] | None = None,
    release_gates: list[str] | None = None,
) -> tuple[RepoProfile, Path, Path, Path]:
    root = project_root.resolve()
...
def refresh_project_skill(profile: RepoProfile) -> Path:
    report = refresh_project_scaffold(profile, render_html=False)
    return Path(report["skill_file"])


def render_project_html(profile: RepoProfile) -> Path:
...
def legacy_html_aliases(profile: RepoProfile) -> list[Path]:
...
def _host_skill_token(profile: RepoProfile) -> str:
...
def _project_skill_bundle(profile: RepoProfile) -> dict[Path, str]:
...
- Regenerate this skill after profile changes with `{cli_command} refresh`.
...
def generate_project_skill(profile: RepoProfile, *, force: bool = False) -> Path:
...
def refresh_project_scaffold(profile: RepoProfile, *, render_html: bool = True) -> dict[str, Any]:
