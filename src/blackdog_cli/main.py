...
from blackdog_core.profile import ConfigError, DEFAULT_SKILL_USAGE_HEURISTIC, load_profile
...
from blackdog.installs import load_tracked_installs, save_tracked_installs
...
from blackdog.tuning import (
    build_prompt_improvement,
    build_prompt_profiles,
    build_tune_analysis,
    seed_tune_task,
)
from blackdog.conversations import (
    append_thread_entry,
    create_thread,
    link_thread_task,
    list_threads,
...
from blackdog.board import UIError, build_board_snapshot
...
COMMAND_SURFACES: dict[str, str] = {
    "create-project": "blackdog",
    "bootstrap": "blackdog",
    "refresh": "blackdog",
    "update-repo": "blackdog",
    "installs": "blackdog",
    "installs add": "blackdog",
    "installs list": "blackdog",
    "installs remove": "blackdog",
    "installs update": "blackdog",
    "installs observe": "blackdog",
    "init": "blackdog_core",
    "backlog": "blackdog_core",
    "backlog new": "blackdog_core",
    "backlog remove": "blackdog_core",
    "backlog reset": "blackdog_core",
    "validate": "blackdog_core",
    "add": "blackdog_core",
    "remove": "blackdog_core",
    "task": "blackdog",
    "task edit": "blackdog",
    "task run": "blackdog",
    "summary": "blackdog_core",
    "plan": "blackdog_core",
    "next": "blackdog_core",
    "snapshot": "blackdog",
    "prompt": "blackdog",
    "thread": "blackdog",
    "thread new": "blackdog",
    "thread list": "blackdog",
    "thread show": "blackdog",
    "thread append": "blackdog",
    "thread prompt": "blackdog",
    "thread task": "blackdog",
    "tune": "blackdog",
    "worktree": "blackdog",
    "worktree preflight": "blackdog_core",
    "worktree start": "blackdog",
    "worktree land": "blackdog",
    "worktree cleanup": "blackdog",
    "supervise": "blackdog",
    "supervise run": "blackdog",
    "supervise sweep": "blackdog",
    "supervise status": "blackdog",
    "supervise recover": "blackdog",
    "supervise report": "blackdog",
    "claim": "blackdog_core",
    "release": "blackdog_core",
    "complete": "blackdog_core",
    "decide": "blackdog_core",
    "comment": "blackdog_core",
    "events": "blackdog_core",
    "render": "blackdog",
...
def _observe_tracked_install(project_root: Path, *, next_limit: int) -> dict[str, Any]:
    profile = load_profile(project_root)
    runtime = load_runtime_artifacts(profile, event_limit=20)
...
def cmd_validate(args: argparse.Namespace) -> int:
    profile = load_profile(Path(args.project_root) if args.project_root else None)
    runtime = load_runtime_artifacts(profile, strict_validate=True)
    payload = {
        ...
        "tasks": len(runtime.backlog.tasks),
        "lanes": len(runtime.backlog.plan.get("lanes", [])),
        "epics": len(runtime.backlog.plan.get("epics", [])),
        "claims": sum(
        ...
        "messages": len(runtime.inbox),
        "open_messages": len([row for row in runtime.inbox if row.get("status") == "open"]),
        "results": len(runtime.results),
        "reconcile": runtime.reconcile_report,
...
def cmd_plan(args: argparse.Namespace) -> int:
    profile, snapshot, state = _load_runtime(Path(args.project_root) if args.project_root else None)
    view = build_plan_snapshot(profile, snapshot, state, allow_high_risk=args.allow_high_risk)
    if args.format == "json":
        print(json.dumps(view, indent=2))
    else:
...
def _build_devtool_parsers(subparsers) -> None:
...
def _build_core_parsers(subparsers) -> None:
    ...
    p_add.add_argument("--epic-id", default=None, help="Legacy compatibility epic id")
    p_add.add_argument("--epic-title", default=None, help="Legacy compatibility epic title")
    p_add.add_argument("--lane-id", default=None, help="Legacy compatibility lane id")
    p_add.add_argument("--lane-title", default=None, help="Legacy compatibility lane title")
    p_add.add_argument("--wave", type=int, default=None, help="Legacy compatibility wave number")
    ...
    p_plan = subparsers.add_parser("plan", help="Show the legacy epic/lane/wave compatibility view from the backlog plan")
...
def _build_blackdog_parsers(subparsers) -> None:
    ...
    p_task_edit.add_argument("--epic-id", default=None, help="Legacy compatibility epic id")
    p_task_edit.add_argument("--epic-title", default=None, help="Legacy compatibility epic title")
    p_task_edit.add_argument("--lane-id", default=None, help="Legacy compatibility lane id")
    p_task_edit.add_argument("--lane-title", default=None, help="Legacy compatibility lane title")
    p_task_edit.add_argument("--wave", type=int, default=None, help="Legacy compatibility wave number")
    ...
    p_thread_task.add_argument("--epic-id", default=None, help="Legacy compatibility epic id")
    p_thread_task.add_argument("--epic-title", default=None, help="Legacy compatibility epic title")
    p_thread_task.add_argument("--lane-id", default=None, help="Legacy compatibility lane id")
    p_thread_task.add_argument("--lane-title", default=None, help="Legacy compatibility lane title")
    p_thread_task.add_argument("--wave", type=int, default=None, help="Legacy compatibility wave number")
...
def _build_worktree_parsers(subparsers) -> None:
...
def build_parser(*, description: str = "Blackdog CLI") -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=description)
    subparsers = parser.add_subparsers(dest="command", required=True)
    _build_devtool_parsers(subparsers)
    _build_core_parsers(subparsers)
    _build_blackdog_parsers(subparsers)
    _build_worktree_parsers(subparsers)
    return parser


def _run_main(argv: list[str] | None = None, *, description: str) -> int:
